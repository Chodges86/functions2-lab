...
// CODE HERE
const postTaxPrices  = prices.map(value => value * 1.07);
...
const ordersWithTax = []
orders.forEach(order => ordersWithTax.push(order.price * (1 + order.tax)))
...
const bobsTotalPrices = bobsPurchases.map(purchaseObject => purchaseObject.price)
const bobsTotal = bobsTotalPrices.reduce((acc, value) => acc + value)
